<?php
#	$Id: folders.inc.php 32 2012-04-05 09:28:38Z along $
#
			...
			function folder_list(){
				...
				for($i = 0; $i < count($folders); $i++) {
					if($folders[$i]['folder_node'] == 1) {
						...
						$a_upload = urr("mydisk","item=upload&folder_node=$folder_node_i&folder_id=$folder_id_i&uid=$pd_uid");
						...
						if(display_plugin('multi_server','open_multi_server_plugin',($settings['open_multi_server'] && $settings['upload_server_type']=='sub_server'),0)){
							...
							if($server_oid>1){
								...
								if($rs){
									$a_upload = $rs['server_host'].'?'.pd_encode("is_public=$is_public&cate_id=$cate_id&subcate_id=$subcate_id&folder_node=$folder_node_i&folder_id=$folder_id_i&uid=$pd_uid&server_key={$rs['server_key']}");
								}
								unset($rs);
							}
						}
						...
						$folder_list .= '<a href="'.$a_share.'" onclick="return hs.htmlExpand(this,{headingText:\''.__('share_folder').'\',objectType:\'iframe\',height:200});" title="'.__('share').'"><img src="images/'.$folder_icon.'.gif" border="0" align="absmiddle"></a>&nbsp;';
						...
						$folder_list .= '<a href="'.$a_upload.'" onclick="return hs.htmlExpand(this,{headingText:\''.__('upload_file').'\',objectType:\'upload\',width:650,height:500});" title="'.__('upload').'"><img src="images/upload_file_icon.gif" border="0" align="absmiddle" /></a>&nbsp;'.LF;
						$folder_list .= '<a href="'.$a_modify.'" onclick="return hs.htmlExpand(this,{headingText:\''.__('folder_modify').'\',objectType:\'iframe\'});" title="'.__('modify').'"><img src="images/edit_icon.gif" border="0" align="absmiddle" /></a>&nbsp;'.LF;
						$folder_list .= '<a href="'.$a_delete.'" onclick="return hs.htmlExpand(this,{headingText:\''.__('folder_delete').'\',objectType:\'iframe\',height:200});" title="'.__('delete').'"><img src="images/recycle_icon.gif" border="0" align="absmiddle" /></a>'.LF;
						...
						for($j = 0; $j < count($folders); $j++) {
							...
							$a_upload = urr("mydisk","item=upload&folder_node=$folder_node_j&folder_id=$folder_id_j&uid=$pd_uid");
							...
							if(display_plugin('multi_server','open_multi_server_plugin',($settings['open_multi_server'] && $settings['upload_server_type']=='sub_server'),0)){
								...
								if($server_oid>1){
									...
									$rs = $db->fetch_one_array("select server_host,server_key from {$tpf}servers where server_oid='$server_oid'limit 1");
									if($rs){
										$a_upload = $rs['server_host'].'?'.pd_encode("is_public=$is_public&cate_id=$cate_id&subcate_id=$subcate_id&folder_node=$folder_node_j&folder_id=$folder_id_j&uid=$pd_uid&server_key={$rs['server_key']}");
									}
									unset($rs);
								}
							}
							...
							if($folders[$j]['parent_id'] == $folders[$i]['folder_id'] && $folders[$j]['folder_node'] == 2) {
								...
								for($k = 0; $k < count($folders); $k++) {
									...
									$a_upload = urr("mydisk","item=upload&folder_node=$folder_node_k&folder_id=$folder_id_k&uid=$pd_uid");
									...
									if(display_plugin('multi_server','open_multi_server_plugin',($settings['open_multi_server'] && $settings['upload_server_type']=='sub_server'),0)){
										...
										if($server_oid>1){
											...
											$rs = $db->fetch_one_array("select server_host,server_key from {$tpf}servers where server_oid='$server_oid'limit 1");
											if($rs){
												$a_upload = $rs['server_host'].'?'.pd_encode("is_public=$is_public&cate_id=$cate_id&subcate_id=$subcate_id&folder_node=$folder_node_k&folder_id=$folder_id_k&uid=$pd_uid&server_key={$rs['server_key']}");
											}
											unset($rs);
										}
									}
									...
									if($folders[$k]['parent_id'] == $folders[$j]['folder_id'] && $folders[$k]['folder_node'] == 3) {
										...
										for($l = 0; $l < count($folders); $l++) {
											...
											$a_upload = urr("mydisk","item=upload&folder_node=$folder_node_l&folder_id=$folder_id_l&uid=$pd_uid");
											...
											if(display_plugin('multi_server','open_multi_server_plugin',($settings['open_multi_server'] && $settings['upload_server_type']=='sub_server'),0)){
												...
												if($server_oid>1){
													...
													$rs = $db->fetch_one_array("select server_host,server_key from {$tpf}servers where server_oid='$server_oid'limit 1");
													if($rs){
														$a_upload = $rs['server_host'].'?'.pd_encode("is_public=$is_public&cate_id=$cate_id&subcate_id=$subcate_id&folder_node=$folder_node_l&folder_id=$folder_id_l&uid=$pd_uid&server_key={$rs['server_key']}");
													}
													unset($rs);
												}
											}
