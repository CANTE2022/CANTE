<?php
#	$Id: files.inc.php 32 2012-04-05 09:28:38Z along $
#
...
switch($action){
		...
		if($task == 'to_folder'){
			...
			function sub_folders($folder_id){
				...
				while($rs = $db->fetch_array($q)){
					...
					$rs['a_upload'] = urr("mydisk","item=upload&folder_node={$rs['folder_node']}&folder_id={$rs['folder_id']}&uid=$pd_uid");
					...
					if(display_plugin('multi_server','open_multi_server_plugin',($settings['open_multi_server'] && $settings['upload_server_type']=='sub_server'),0)){
						...
						if($server_oid>1){
							// reload $a_upload_file
							$rs2 = $db->fetch_one_array("select server_host,server_key from {$tpf}servers where server_oid='$server_oid'limit 1");
							if($rs2){
								$rs['a_upload'] = $rs2['server_host'].'?'.pd_encode("is_public=$is_public&cate_id=$cate_id&subcate_id=$subcate_id&folder_node={$rs['folder_node']}&folder_id={$rs['folder_id']}&uid=$pd_uid&server_key={$rs2['server_key']}");
							}
							unset($rs2);
						}
					}
	...
	case 'extract_modify':
		...
		if($task =='extract_modify'){
			...
			function list_cur_extract($extract_id){
				...
				unset($rs);
				$q = $db->query("select * from {$tpf}files where userid='$pd_uid' and in_recycle=0 and file_id in ($file_str) order by file_id desc");
				$files_array = array();
				while($rs = $db->fetch_array($q)){
					if($rs['folder_id']){
						$rs2 = $db->fetch_one_array("select folder_name from {$tpf}folders where folder_id='{$rs['folder_id']}'");
	...
	case 'make_link_code':
		...
		if($file_str){
			$q = $db->query("select file_id,file_name,file_key,file_extension,is_image,file_store_path,file_real_name,store_old,server_oid from {$tpf}files where file_id in ($file_str) order by file_id $order");
			$upl_array = array();
			while($rs = $db->fetch_array($q)){
				$tmp_ext = $rs['file_extension'] ? '.'.$rs['file_extension'] : "";
				$rs['file_name_all'] = $rs['file_name'].$tmp_ext;
			...
			$db->free($q);
			unset($rs);
			$order_url = $settings['phpdisk_url'].urr("mydisk","item=$item&action=$action&file_ids=$file_str&order=$order_txt");
