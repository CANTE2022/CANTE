...
$arr = array('global','plugin','ftp','cache','image');
...
if(!$pd_uid || !$pd_pwd){
	...
	$userinfo = $db->fetch_one_array("select userid,u.gid,username,password,email,group_name,is_activated from {$tpf}users u,{$tpf}groups g where username='$pd_username' and password='$pd_pwd' and u.gid=g.gid limit 1");
	if($userinfo){
...
$news_url = $auth['com_news_url'] ? $auth['com_news_url'] : 'http://www.phpdisk.com/m_news/vcore_idx.php';
$upgrade_url = $auth['com_upgrade_url'] ? $auth['com_upgrade_url'] : 'http://www.phpdisk.com/autoupdate/vcore_last_version.php';
