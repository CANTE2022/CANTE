...
#
#	$Id: global.func.php 42 2012-06-08 08:26:21Z along $
#
...
function template_parse($tpl){
	...
	$prefix = "<?php ".LF;
	$prefix .= "// 海诚Ｅ盘（pan.haic.cc）_网络存储系统".LF.LF;
...
function get_size1($s,$u='B',$p=1){
	return number_format($s/1024/1024,1);
}
...
function delete_phpdisk_file($sql){
			...
			if(display_plugin('multi_server','open_multi_server_plugin',$settings['open_multi_server'],0) && $rs['server_oid']>1){
				if($rs['store_old']){
					...
					if($rs3){
						...
						pftp_chdir($ftp_res,$rs3['server_store_path']);
						pftp_delete($ftp_res,$rs['file_store_path'].'/'.$rs['file_real_name']);
						pftp_delete($ftp_res,$rs['file_store_path'].'/'.$rs['file_real_name'].'_thumb');
						pftp_close($ftp_res);
					}
					unset($rs3);
				}else{
...
function get_file_thumb($rs){
	...
	if(display_plugin('multi_server','open_multi_server_plugin',$settings['open_multi_server'],0) && $rs['server_oid'] >1){
		...
		if($rs2){
			if($rs['is_image']){
				if($rs['store_old']){
					$file_thumb = $rs2['server_host'].$rs2['server_store_path'].'/'.$rs['file_store_path'].'/'.$rs['file_real_name'].'_thumb';
				}else{
