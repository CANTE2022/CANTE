...
$syxx = htmlspecialchars_decode($settings['syxx']);
...
$last = super_cache::get('index_last','user');
$C[announce_arr] = super_cache::get('get_announce_list','announce',1,0);
$C[gallery_arr] = super_cache::get('get_index_galley','gallery',1,0);

$C[hot_tags] = get_hot_tags();
$C[last_file] = super_cache::get('get_last_file','file');
$C[hot_file] = super_cache::get('get_hot_file','file');

$C[links_arr] = super_cache::get('get_friend_link','link',1,0);
main_stats();
$last_users = super_cache::get('get_last_user_list');
...
function get_last_user_list(){
	global $db,$tpf;
	$q = $db->query("select username,reg_time from {$tpf}users order by userid desc limit 15");
	$last_users = array();
	while ($rs = $db->fetch_array($q)) {
