...
$in_front = true;
$title = '用户注册 '.$settings['site_title'];
...
switch($action){
	...
	case 'login':
		...
		if($task =='login'){
			...
			$username = $username_pd = gpc('dl1','P','',1);
			$password = $password_pd = gpc('dl2','P','',1);
			$md5_pwd = md5($password);
			$remember = (int)gpc('remember','P',0);
			$verycode = trim(gpc('dl3','P','',1));
			$ref = trim(gpc('ref','P',''));
...
*/			$rs = $db->fetch_one_array("select userid,gid,username,password,email from {$tpf}users where username='$username' limit 1");
		...
		require_once template_echo('pd_adminlogin',$user_tpl_dir);
		break;

	case 'register':
		$title = __('account_register');
		if($task == 'register'){

			form_auth(gpc('formhash','P',''),formhash());

			$username = trim(gpc('tedlmc','P','',1));
			$password = trim(gpc('temm','P','',1));
			$confirm_password = trim(gpc('temmqr','P',''));
			$email = trim(gpc('teem','P','',1));
			$verycode = trim(gpc('verycode','P','',1));
...
*/			if(!$error && display_plugin('api','open_uc_plugin',$settings['connect_uc'],0)){
			...
			if(!$error){
				...
				'email' => $email,
				'gid' => $gid,
				'is_activated' => $settings['user_active']==0 ? 1 : 0,
				'credit' => $credit,
...
//				exit;
			...
			require_once template_echo('pd_register',$user_tpl_dir);
		}

		break;

	case 'forget_pwd':
		$title = __('account_forget_pwd');
		if($task =='forget_pwd'){
			form_auth(gpc('formhash','P',''),formhash());
